...
/*   Updated: 2024/06/20 18:58:43 by albartol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	extract_identifiers(t_scene_file *info)
{
	t_list	*temp;
	size_t	len;
	int		list_pos;
	...
	while (temp)
	{
		...
		if (len >= 1 && check_identifiers(info, temp->content, len))
		{
			...
			ft_putstr_fd(" : is invalid in a scene file\n", STDERR_FILENO);
			return (0);
		}
		if (errno == ENOMEM)
			return (0);
...
int	extract_scene_info(t_scene_file *scene_info)
{
	...
	list_pos = extract_identifiers(scene_info);
	...
	{
		if (errno != ENOMEM)
			print_error("Invalid or missing identifiers in scene file\n");
		return (EXIT_FAILURE);
	}
	return (EXIT_SUCCESS);
}
